...
class TestCashReminder:
    ...
    def test_on_route_fires_reminder_once(self):
        ...
        # R67 — Twilio SMS was removed from the cash-reminder pipeline.
        # The customer is now notified via push (above) + email (above) only.
        # Any future channel (APNs / FCM push) will get its own assertion.
        assert db.sms_log.count_documents({"booking_id": bkg_id}) == 0
