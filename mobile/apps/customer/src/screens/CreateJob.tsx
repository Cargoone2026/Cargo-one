/**
 * CreateJobScreen — booking form. The full 5-step web wizard requires
 * native AddressAutocomplete + RouteMap + PhotoUpload components,
 * which land in a follow-up commit. This screen keeps the same fields
 * but styles them with the shared Cargo One design system.
 */
...
export function CreateJobScreen({ route, navigation }: P) {
  ...
  const kicker = isAsap ? `ASAP ${serviceType}` : isBid ? "Big Job / Bidding" : "Fixed Price";

  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <Page testID="create-job-screen">
        <ScrollView keyboardShouldPersistTaps="handled">
          <PageHeader
            title={
              <View style={{ gap: 4 }}>
                <Text style={typography.micro}>{kicker.toUpperCase()}</Text>
                <Text style={typography.h1}>New booking</Text>
              </View>
            }
            subtitle="Enter your pickup and destination. Pricing follows the Cargo One rules."
          />
          <View style={{ paddingHorizontal: 16, paddingBottom: 40 }}>
            ...
            <View style={{ marginTop: 20 }}>
              <PrimaryButton
                title={isAsap ? "Book now" : isBid ? "Post for bids" : "Post fixed-price job"}
                onPress={onSubmit}
                loading={busy}
                testID="job-submit"
              />
