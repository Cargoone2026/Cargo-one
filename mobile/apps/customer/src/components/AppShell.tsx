/**
 * AppShell — native equivalent of frontend/src/components/portal/SideRail
 * + PortalShell. Renders the CargoOne dark sidebar (bg #0B0B0F) as
 * either:
 *
 *   • Docked (width ≥ 900):   sidebar sits permanently on the left.
 *                              A collapse toggle folds it to a compact
 *                              icon rail (72 px) and back to the full
 *                              240 px width, animated.
 *   • Drawer (width <  900):  sidebar is a slide-in drawer with a
 *                              scrim. A menu button top-left of the
 *                              main content opens it.
 *
 * There is NO bottom tab bar. The full CargoOne navigation is always
 * present, matching web parity.
 */
...
const NAV: { label: string; route: keyof RootStackParamList; Icon: any }[] = [
  ...
  { label: "Post Job", route: "PostJob", Icon: PlusCircle },
  { label: "ASAP", route: "Asap", Icon: Zap },
  { label: "Bookings", route: "Bookings", Icon: Package },
...
export const AppShellContext = React.createContext<{
  openDrawer: () => void;
  showMenu: boolean;
}>({ openDrawer: () => {}, showMenu: false });

/**
 * useShellMenu — screens call this to decide whether to render a menu
 * button in their PageHeader and how to open the drawer. When docked
 * (≥900 px), the sidebar is always visible so no menu button is
 * needed and `showMenu` is false.
 */
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const shellCtx = React.useMemo(
    () => ({ openDrawer: () => setOpen(true), showMenu: !docked }),
    [docked],
  );
...
function SidebarContent({
  ...
  return (
    <SafeAreaView edges={["top", "bottom"]} style={styles.sidebar}>
      {/* Brand */}
      <Pressable onPress={() => go("Home")} style={styles.brand} testID="sidebar-brand">
